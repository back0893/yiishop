...
$this->registerCssFile('/style/order.css');
	...
	<div class="main w1210 bc mt10">
		...
		<div class="content fl ml10">
			<div class="order_hd">
				...
				<dl>
					<dt>便利提醒：</dt>
                    <dd>待付款（<?=isset($queries[1])?$queries[1]:0?>）</dd>
					<dd>待确认收货（<?=isset($queries[2])?$queries[2]:0?>）</dd>
					<dd>待自提（<?=isset($queries[3])?$queries[3]:0?>）</dd>
				</dl>
				...
				<table class="orders">
                    ...
                    <?php foreach ($orders as $order):?>
                        <tr>
                            ...
                            <td>
                                <?php foreach($order->orderGoods as $n=>$goods):
                                    if($n>2){
                                        break;
                                    }
                                    $n++;
                                ?>
                                    <a href="<?=Url::to(['index/goods','id'=>$goods->goods_id])?>"><img src="<?=$goods->logo?>" alt="<?=$goods->goods_name?>" /></a>
                                <?php endforeach;?>
                            </td>
                            <td><?=$order->name?></td>
                            <td><?php
                                printf('￥ %.2f %s',$order->total,$order->payment_name);
                                ?>
                            </td>
                            <td><?=date('Y-m-d H:m:i',$order->create_time)?></td>
                            <td><?=$status[$order->status]?>
                            </td>
                            <td><a href="<?=Url::to(['cart/order-detail','id'=>$order->id])?>">查看</a> | <a href="">删除</a></td>
                        </tr>
                    <?php endforeach;?>
					</tbody>
				</table>
