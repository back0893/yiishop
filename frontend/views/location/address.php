...
$this->registerJsFile('/js/home.js',['depends'=>'yii\web\JqueryAsset']);
?>
	<!-- 头部 start -->
	...
	<div class="main w1210 bc mt10">
		...
		<div class="content fl ml10">
			<div class="address_hd">
				...
				<dl data-id="<?=$address->id?>">
					<dt>
                        <?php
                        $temp=$index+1;
                        $addr="{$temp} .$address->name. ".$address->getAddress('province')->name.' '. $address->getAddress('city')->name.' '.$address->getAddress('town')->name.' '."$address->address $address->tel";
                        echo $addr;
                        ?></dt>
					<dd>
                        ...
                        <?php if($address->status==0):?>
						    <a href="javascript:void(0)" class="default_addr">设为默认地址</a>
                        <?php else:?>
                            <a id="default">默认地址</a>
                        <?php endif;?>
					</dd>
				...
				<form method="post" name="address_form" id="address_form">
                    ...
                    <input type="hidden" value="0" name="id"/>
						<ul>
							...
							<li>
								<label for="">&nbsp;</label>
                                <input type="hidden" name="status" class="check" value="0" />
...
<script>
    ...
    var getinfo='<?=\yii\helpers\Url::to(['location/getinfo'])?>';
    var de='<?=\yii\helpers\Url::to(['location/default'])?>';
    var del='<?=\yii\helpers\Url::to(['location/del'])?>';
    var edit='<?=\yii\helpers\Url::to(['location/edit'])?>';
</script>
<?php
...
$this->registerJsFile('/myjs/address.js',['depends'=>'yii\web\JqueryAsset']);
?>
</body>
</html>